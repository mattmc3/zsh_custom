...
function editor-info {
  ...
  # are zsh_custom managed and/or compatible.
  if zstyle -t ':zsh_custom:plugin:prompt' managed; then
    # Clean up previous $editor_info.
    unset editor_info
    typeset -gA editor_info

    if [[ "$KEYMAP" == 'vicmd' ]]; then
      zstyle -s ':zsh_custom:plugin:editor:info:keymap:alternate' format 'REPLY'
      editor_info[keymap]="$REPLY"
    else
      zstyle -s ':zsh_custom:plugin:editor:info:keymap:primary' format 'REPLY'
      editor_info[keymap]="$REPLY"

      if [[ "$ZLE_STATE" == *overwrite* ]]; then
        zstyle -s ':zsh_custom:plugin:editor:info:keymap:primary:overwrite' format 'REPLY'
        editor_info[overwrite]="$REPLY"
      else
        zstyle -s ':zsh_custom:plugin:editor:info:keymap:primary:insert' format 'REPLY'
        editor_info[overwrite]="$REPLY"
      fi
    fi
...
function zle-reset-prompt {
  if zstyle -t ':zsh_custom:plugin:editor' ps-context; then
...
function expand-or-complete-with-indicator {
  local indicator
  zstyle -s ':zsh_custom:plugin:editor:info:completing' format 'indicator'
...
function _zsh_custom-zle-noop {  ; }
zle -N _zsh_custom-zle-noop
...
for keymap in $unbound_keys; do
  bindkey -M viins "${keymap}" _zsh_custom-zle-noop
  bindkey -M vicmd "${keymap}" _zsh_custom-zle-noop
done
...
if [[ "$key_bindings" == (emacs|) ]]; then
  ...
  print "zsh_custom: editor: invalid key bindings: $key_bindings" >&2
fi
