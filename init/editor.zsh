...
source $MY_ZSH_CUSTOM/pkg/zsh-no-ps2.zsh
...
zstyle -s ':zsh_custom:editor' wordchars 'WORDCHARS' || \
WORDCHARS='*?_-.[]~&;!#$%^(){}<>'
...
function update-cursor-style {
  ...
  zstyle -s ":zsh_custom:editor:$KEYMAP" cursor style
  if [[ -z "$style" ]]; then
    case "$KEYMAP" in
...
function glob-alias {
  ...
  zstyle -a ':zsh_custom:editor:glob-alias' 'noexpand' 'noexpand_aliases' \
    || noexpand_aliases=()
...
function magic-enter-cmd {
  local cmd
  zstyle -s ':zsh_custom:editor:magic-enter' command 'cmd' ||
    cmd="ls ."

  if command git rev-parse --is-inside-work-tree &>/dev/null; then
    zstyle -s ':zsh_custom:editor:magic-enter' git-command 'cmd' ||
      cmd="git status -sb ."
  fi
  echo $cmd
}
...
if zstyle -t ':zsh_custom:editor' 'magic-enter'; then
...
for _zph_feature _zph_key in ${(kv)_zph_opt_in_keybinds}; do
  if zstyle -t ':zsh_custom:editor' "$_zph_feature"; then
...
if zstyle -t ':zsh_custom:editor' dot-expansion; then
  bindkey -M isearch . self-insert 2> /dev/null
fi

# Expand aliases with space automatically (opt-in, overrides glob-alias)
if zstyle -t ':zsh_custom:editor' automatic-glob-alias; then
  for _zph_keymap in 'emacs' 'viins'; do
...
elif zstyle -T ':zsh_custom:editor' glob-alias; then
  for _zph_keymap in 'emacs' 'viins'; do
    bindkey -M "$_zph_keymap" "^ " glob-alias
    bindkey -M "$_zph_keymap" " " magic-space
  done
...
# Set the key layout.
zstyle -s ':zsh_custom:editor' key-bindings '_zph_key_bindings'
