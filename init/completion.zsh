#
# completion: Plugin that makes it much easier to initialize Zsh completions.
#
...
function run-compinit {
  ...
  if [[ "$1" == (-f|--force) ]]; then
    shift
    [[ -r "$zcompdump" ]] && rm "$zcompdump"
  fi

  # Initialize completions
  local -a compinit_flags=(-d "$zcompdump")
  autoload -Uz compinit
  if zstyle -T ':zsh_custom:init:compinit' 'use-cache'; then
...
function run-compstyleinit {
  compstyleinit
  local -a mycompstyle
  if zstyle -a ':zsh_custom:init:compinit' 'compstyle' mycompstyle; then
...
if zstyle -t ':zsh_custom:init:compinit' compstyle-defer; then
